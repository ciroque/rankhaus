...
impl MergeStrategy {
    ...
    fn initialize_merge_sort(&mut self) {
        ...
        let mut sublists: Vec<Vec<Id>> = self.items.iter().map(|id| vec![id.clone()]).collect();
    ...
    fn get_winner<'a>(&self, a: &'a Id, b: &'a Id) -> Option<&'a Id> {
        ...
        self.comparisons
            .get(&key)
            .map(|winner| if winner == &a.to_string() { a } else { b })
    }
}
    ...
    fn process_merges(&mut self) {
        ...
        for idx in 0..self.state.merge_stack.len() {
            ...
            if let Some(source_idx) = left_source {
                if self.state.merge_stack[source_idx].left_idx
                    == self.state.merge_stack[source_idx].left.len()
                    && self.state.merge_stack[source_idx].right_idx
                        == self.state.merge_stack[source_idx].right.len()
                    && !self.state.merge_stack[source_idx].result.is_empty()
                {
                    ...
                    if self.state.merge_stack[idx].left_idx == 0
                        && self.state.merge_stack[idx].result.is_empty()
                    {
            ...
            if let Some(source_idx) = right_source {
                    ...
                    && !self.state.merge_stack[source_idx].result.is_empty()
                {
                    let result = self.state.merge_stack[source_idx].result.clone();
                    // Only update if we haven't started this merge yet
                    if self.state.merge_stack[idx].right_idx == 0
                        && self.state.merge_stack[idx].result.is_empty()
                    {
...
mod tests {
    ...
    fn test_two_items() {
        ...
        // Make the comparison
        strategy
            .compare(&items[0], &items[1], &items[0].id)
            .unwrap();
    ...
    fn test_six_items() {
        ...
        println!(
            "Merge stack has {} operations:",
            strategy.state.merge_stack.len()
        );
        ...
        while let Some((a_id, b_id)) = strategy.next_comparison() {
            ...
            println!(
                "Comparison {}: {} vs {} -> {}",
                comparison_count, a.value, b.value, winner.value
            );
            println!("  Complete: {}", strategy.is_complete());

            // Debug: show which operations are ready
            let ready_ops: Vec<usize> = strategy
                .state
                .merge_stack
                .iter()
                .enumerate()
