...
fn remove(state: Option<&mut AppState>, identifier: String, cascade: bool) -> Result<()> {
    ...
    // Check for rankings
    let has_rankings = rankset.rankings.iter().any(|r| r.user_id.as_str() == user_id);
...
fn edit(state: Option<&mut AppState>, identifier: String, new_display_name: String) -> Result<()> {
    ...
    let user = rankset.get_user_mut(&identifier)
    ...
    println!("✓ Updated display name: '{}' → '{}'", old_display, new_display_name);
...
fn select(state: Option<&mut AppState>, identifier: String) -> Result<()> {
    ...
    let rankset = app_state.rankset.as_ref()
        .ok_or_else(|| anyhow::anyhow!("No rankset loaded"))?;
...
mod tests {
    ...
    fn test_edit_user() {
        let mut state = create_test_state();
        let result = edit(Some(&mut state), "alice".to_string(), "Alice Smith".to_string());
        assert!(result.is_ok());
    ...
    fn test_edit_user_by_id() {
        let mut state = create_test_state();
        let user_id = state.rankset.as_ref().unwrap()
            .users.values().next().unwrap().id.to_string();
    ...
    fn test_edit_user_not_found() {
        let mut state = create_test_state();
        let result = edit(Some(&mut state), "nonexistent".to_string(), "Name".to_string());
        assert!(result.is_err());
    }
