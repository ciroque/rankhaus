...
pub fn execute(command: ItemsCommands, state: Option<&mut AppState>) -> Result<()> {
    ...
    match command {
        ItemsCommands::List => list(state),
        ItemsCommands::Add { item } => add(state, item),
        ItemsCommands::Remove => remove(state),
        ItemsCommands::Edit {
            identifier,
            new_value,
...
mod tests {
    use super::*;
    use rankhaus::{Item, RankSet, User};
    use std::path::PathBuf;

    fn create_test_state() -> AppState {
        let mut list = RankSet::new("test".to_string(), "author".to_string(), None);
    ...
    fn test_list_with_items() {
        ...
        state
            ...
            .as_mut()
            .unwrap()
            .add_item(Item::new("red".to_string()))
            .unwrap();
        state
            .rankset
            .as_mut()
            .unwrap()
            .add_item(Item::new("blue".to_string()))
            .unwrap();
