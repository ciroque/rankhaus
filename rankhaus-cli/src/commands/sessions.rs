...
pub fn execute(command: SessionsCommands, state: Option<&mut AppState>) -> Result<()> {
    ...
    match command {
        SessionsCommands::List => list(state),
        SessionsCommands::Show { session_id } => show(state, session_id),
        SessionsCommands::Delete { session_id } => delete(state, session_id),
        SessionsCommands::Resume { session_id } => rank::resume(session_id, state),
    }
}
...
fn list(state: Option<&mut AppState>) -> Result<()> {
    ...
    println!(
        "{:<10} {:<12} {:<10} {:<10} {:<15}",
        "Session", "User", "Strategy", "Status", "Created"
    );
    println!("{:-<60}", "");
...
fn show(state: Option<&mut AppState>, session_id: String) -> Result<()> {
    ...
    println!(
        "Created:      {}",
        ranking.session.info.created.format("%Y-%m-%d %H:%M:%S UTC")
    );
    println!(
        "Last Updated: {}",
        ranking
